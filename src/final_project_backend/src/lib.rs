...
thread_local! {
    ...
    static PROPOSAL_MAP: RefCell<StableBTreeMap<u64, Proposal, Memory>> = RefCell::new(StableBTreeMap::init(MEMORY_MANAGER.with(|mm| mm.borrow().get(MemoryId::new(0)))));
}
...
fn edit_proposal(key: u64, proposal: CreateProposal) -> Result<(), VoteError> {
    PROPOSAL_MAP.with(| p | {
        ...
        let result =  p.borrow_mut().insert(key, new_proposal));
        ...
        // let mut proposal = p.borrow_mut().get_mut(&key).ok_or(VoteError::NoSuchProposal)?;
        // proposal.description = proposal.description;
        // proposal.is_active = proposal.is_active;
        // Ok(())
    })
